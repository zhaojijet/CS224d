...
class RNN_Model():
    ...
    def add_model(self, node):
        ...
        if node.isLeaf:
            ...
            ### YOUR CODE HERE
            below_node_tenser = tf.concat(1, [node_tensors[node.left], node_tensors[node.right]])
            curr_node_tensor = tf.nn.relu(tf.matmul(below_node_tenser, W1) + b1)
            ### END YOUR CODE
    ...
    def loss(self, logits, labels):
        ...
        entropy_loss = tf.reduce_sum(tf.nn.sparse_softmax_cross_entropy_with_logits(logits, labels))
        loss = entropy_loss + self.config.l2 * l2_loss
        # END YOUR CODE
        return loss
