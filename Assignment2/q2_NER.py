...
class NERModel(LanguageModel):
  ...
  def add_embedding(self):
    ...
    with tf.device('/cpu:0'):
      ...
      #embeddings_init = xavier_weight_init()
      #embeddings = embeddings_init((len(self.wv), self.config.embed_size))
  ...
  def add_model(self, window):
    ...
    with tf.variable_scope("Layer", initializer = xavier_weight_init()):
        W = tf.get_variable("Layer-weights", (self.config.window_size * self.config.embed_size, self.config.hidden_size))
        b1 = tf.get_variable("Layer-bias", (self.config.hidden_size, ))
        y_Layer = tf.nn.tanh(tf.matmul(window, W) + b1)
        # Regularization
        if self.config.l2:
    ...
    with tf.variable_scope("Softmax", initializer = xavier_weight_init()):
        U = tf.get_variable("Softmax-weights", (self.config.hidden_size, self.config.label_size))
        b2 = tf.get_variable("Softmax-bias", (self.config.label_size, ))
        y_Softmax = tf.matmul(y_Layer, U) + b2
        # Regularization
        if self.config.l2:
            tf.add_to_collection('total_loss', 0.5 * self.config.l2 * tf.nn.l2_loss(U))
    output = tf.nn.dropout(y_Softmax, self.dropout_placeholder)
    ### END YOUR CODE
    return output 
  ...
  def add_loss_op(self, y):
    ...
    """
    ### YOUR CODE HERE
    cross_entropy_loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(y, self.labels_placeholder))
    # loss
