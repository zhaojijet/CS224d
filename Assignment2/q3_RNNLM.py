...
class RNNLM_Model(LanguageModel):
  ...
  def add_embedding(self):
    ...
    with tf.device('/cpu:0'):
        ...
        # inputs.shape: (len(self.input_placeholder), self.config.embed_size)
        inputs = tf.nn.embedding_lookup(embeddings, self.input_placeholder)
        # split: inputs.shape: (len(self.input_placeholder), self.config.embed_size) --> (len(self.input_placeholder), 1, self.config.num_steps)
        # squeeze: inputs.shape: (len(self.input_placeholder), 1, self.config.num_steps) --> (len(self.input_placeholder, self.config.num_steps))
  ...
  def add_projection(self, rnn_outputs):
    ...
    with tf.variable_scope("projection"):
        U = tf.get_variable("RNN-U", (self.config.hidden_size, len(self.vocab)))
        b_2 = tf.get_variable("RNN-b_2", (len(self.vocab), ))
  ...
  def add_model(self, inputs):
    ...
    with tf.variable_scope("RNN") as scope:
        self.initial_state = state = tf.zeros([self.config.batch_size, self.config.hidden_size])
        rnn_outputs = []
        for index, current_input in enumerate(inputs):
            if index > 0:
                scope.reuse_variables()
            H = tf.get_variable("RNN-H", (self.config.hidden_size, self.config.hidden_size))
            I = tf.get_variable("RNN-I", (self.config.embed_size, self.config.hidden_size))
            b_1 = tf.get_variable("RNN-b_1", (self.config.hidden_size, ))
            state = tf.sigmoid(tf.matmul(state, H) + tf.matmul(current_input, I) + b_1)
            rnn_outputs.append(state)
        self.final_state = rnn_outputs[-1]
    with tf.variable_scope("output"):
        rnn_outputs = [tf.nn.dropout(y, self.dropout_placeholder) for y in rnn_outputs]
    ### END YOUR CODE
    return rnn_outputs
